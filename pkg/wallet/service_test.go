...
func TestService_Export(t *testing.T) {
	wallet := NewTestService()
	_, _, err := wallet.addAccount("+212124", 100)
	_, _, err = wallet.addAccount("+23", 100)
	_, _, err = wallet.addAccount("+211232124", 100)
	if err != nil {
		t.Errorf("Service_Export(): Error: can't add account: %v", err)
	}
	_, err = os.Create("text.txt")
	if err != nil {
		t.Errorf("Service_Export(): Error: can't open file: %v", err)
	}
	err = wallet.ExportToFile("text.txt")
	if err != nil {
		return
	}
}
...
func TestImportFromFile(t *testing.T) {
	...
	if err != nil {
		t.Errorf("ImportFromFile(): Error: can't open file: %v", err)
	}
	defer file.Close()

	err = wallet.ImportFromFile("text.txt")
	if err != nil {
		t.Errorf("ImportFromFile(): Error: import not working: %v", err)
	}
	if len(wallet.accounts) != 3 {
		t.Error("ImportFromFile(): Error: unsuccess import")
	}
}
